...
export class E2BBrowserSession implements BrowserSession {
  ...
  private async startServer() {
    ...
    const command = [
      ...
      `  setsid -f ${shellSingleQuote(NODE_BINARY)} ${shellSingleQuote(SERVER_PATH)} > ${shellSingleQuote(SERVER_LOG_PATH)} 2>&1 < /dev/null`,
      "else",
      `  nohup ${shellSingleQuote(NODE_BINARY)} ${shellSingleQuote(SERVER_PATH)} > ${shellSingleQuote(SERVER_LOG_PATH)} 2>&1 < /dev/null &`,
      "fi",
