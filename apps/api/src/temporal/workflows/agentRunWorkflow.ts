...
const { startScheduledRunActivity } = proxyActivities<typeof activities>({
  retry: {
    initialInterval: "5 seconds",
    maximumAttempts: 1,
  },
...
export async function scheduledRunWorkflow(input: ScheduledRunWorkflowInput) {
  return startScheduledRunActivity(input);
}
